...
func loadConfig(file string, cfg *gethConfig) error {
	...
	if err != nil {
		log.Error("Error while reading full config file", "err", err)
	}
...
func makeConfigNode(ctx *cli.Context) (*node.Node, gethConfig) {
	...
	// Load config file.
	log.Info(fmt.Sprintf("Checking for config file"))
	if file := ctx.String(configFileFlag.Name); file != "" {
		log.Info(fmt.Sprintf("Checking for config file to load: %v", file))
		if err := loadConfig(file, &cfg); err != nil {
			utils.Fatalf("%v", err)
		}
		log.Info(fmt.Sprintf("Loaded config file: %v", file))
	} else {
